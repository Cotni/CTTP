...
void LaunchSoftware(){ //If dolphin, restarts game, else launches Riivo->HBC->OHBC->WiiMenu
    s32 result = CTTP::Open("/dev/dolphin", IOS::MODE_NONE);
    if(result >= 0){
        IOS::Close(result);
        SystemManager::RestartGame();
        return;
    }
    result = CTTP::Open("/title/00010001/52494956/content/title.tmd\0", IOS::MODE_NONE); //Riivo
    if (result >= 0){
        ISFS::Close(result);
        OSLaunchTitle(0x00010001, 0x52494956);
        return;
    }
    result = CTTP::Open("/title/00010001/4c554c5a/content/title.tmd\0", IOS::MODE_NONE); //OHBC
    if (result >= 0){
        ISFS::Close(result);
        OSLaunchTitle(0x00010001, 0x4c554c5a);
        return;
    }
    result = CTTP::Open("/title/00010001/48424330/content/title.tmd\0", IOS::MODE_NONE); // If HBC can't be found try OHBC
    if (result >= 0){
        ISFS::Close(result);
...
void CreateCrashFile(){
    ...
    if(fileHandler != NULL){ //should always exist if the crash is after strap 
        ...
        snprintf(path, IPCMAXPATH, "%s/CTTPCrash.bin", CTTP::modFolder);
