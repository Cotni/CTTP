...
int GetCurTrackBMG(){
   return CTTP::GetCourseId() + BMG_TRACKS;
}
   ...
   for(int i = 0; i<4; i++){
      ...
      if(lastSelectedCup * 4 + i == selectedCourse){
         ...
         if(RaceData::sInstance->menusScenario.settings.gamemode == MODE_TIME_TRIAL){
            TextInfo text;
            text.bmgToPass[0] = BMG_TT_MODE_BOTTOM_CUP + CTTP::ttMode;
            u32 bmgId = BMG_NO_TROPHY;
            TrackTrophy *trophy = SIPDataHolder::GetInstance()->FindTrackTrophy(CTTP::GetCRC32((CourseId)curButton->buttonId), CTTP::ttMode);
            if(trophy != NULL && trophy->hastrophy[CTTP::ttMode]) bmgId = BMG_TROPHY;
