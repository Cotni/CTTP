...
#include <SlotExpansion/UI/CTTPCupSelect.hpp>
...
UIControl *ExpandedCupSelect::CreateControl(u32 controlId){
   if(controlId == 2){
      ...
      arrows.Load(CTTP::buttonFolder, "SIPCupsArrowRight", "ButtonArrowRight", 
      ...
      loader.Load(CTTP::controlFolder, "RandomTrackCup", "RandomControls", NULL);
...
void ExpandedCupSelect::ChangeCupTPL(LayoutUIControl *control, u32 id){
   ...
   CTTP::ChangeImage(control, "icon",          tplName);
   CTTP::ChangeImage(control, "icon_light_01", tplName);
   CTTP::ChangeImage(control, "icon_light_02", tplName);
}
...
void ExpandedCupSelect::OnStartPress(u32 hudSlotId){
   if(RaceData::sInstance->menusScenario.settings.gamemode == MODE_TIME_TRIAL && this->randomizedId == -1){
      ...
      for(int i = 0; i<TRACKCOUNT; i++){
         if(!data->HasTrophy((CourseId) i, CTTP::ttMode)){
...
void ExpandedCupSelect::AfterControlUpdate(){
   ...
   if(RaceData::sInstance->menusScenario.settings.gamemode == MODE_TIME_TRIAL){
      ...
      u32 trophyCount = SIPDataHolder::GetInstance()->GetTrophyCount(CTTP::ttMode);
