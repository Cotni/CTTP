...
CourseId CTTP::GetCourseId(){
   if(CTTP::winningCourse >= 0x44) return (CourseId) (CTTP::winningCourse - 0x44);
   else return (CourseId) CTTP::winningCourse;
}
...
void SaveSelectedCourse(PushButton *button){
   ...
   CTTP::winningCourse = selectedCourse + 0x44; //+0x44 for online stuff, since slot 0x43 is used by the game for "hasn't voted yet"
}

//Converts trackID to track slot using table
CourseId GetCorrectTrackSlot(){
   CourseId id = CTTP::GetCourseId();
   return (CourseId) CTTP::extSlotToTrackId[id / 4][id % 4];
}
...
int GetCorrectMusicSlot(){
   ...
   if(id <= 0x1F){
      id = CTTP::GetCourseId();
      id = (CourseId) CTTP::extSlotToMusicSlot[id/4][id % 4];   
   }
...
void FormatTrackPath(char *path, u32 length, const char *format, const char *fileName){
   ...
   if(gamemode != MODE_BATTLE && gamemode != MODE_PUBLIC_BATTLE && gamemode != MODE_PRIVATE_BATTLE && gamemode != MODE_AWARD){
      snprintf(path, 0x80, "Race/Course/%d", CTTP::GetCourseId());
   } 
...
void FixGrandPrix(Pages::CupSelect *page, MenuId nextMenu, PushButton *button){
   CTTP::winningCourse = button->buttonId * 4 + 0x44;
   page->ChangeMenuById(VS_RACE_PANORAMA, button);
};
//kmCall(0x80841854, FixGrandPrix);

/*
using CTTP::extSlotToTrackId;
using CTTP::winningCourse;
...
RacedataScenario *UseCorrectCourse(RacedataScenario *scenario){
   CTTP::winningCourse = lastSelectedCup * 4 + scenario->settings.raceNumber + 0x44;
...
void VSRaceRandomFix(MenuData98 *m98){ //properly randomizes tracks and sets the first one
   ...
   CTTP::winningCourse = m98->vsTracks[0] + 0x44;
...
void VSRaceOrderedFix(MenuData98 *m98){
   m98->vsRaceLimit = 32;
   CourseId initial = CTTP::GetCourseId();
