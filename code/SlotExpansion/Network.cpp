...
void DecideTrack(RKNetSELECTHandler *select){
    ...
    if(select->mode == ONLINEMODE_PRIVATE_VS && SIPDataHolder::GetInstance()->GetSettings()->pages[CTTP_HOST_SETTINGS].radioSetting[CTTP_RADIO_HOSTWINS] == CTTP_RADIO_HOSTWINS_ENABLED){
...
void CorrectRH1(PacketHolder *packetHolder, RACEHEADER1Packet *rh1Packet, u32 len){
    rh1Packet->trackId = (u8) CTTP::winningCourse;
...
CourseId ReturnCorrectId(RKNetRH1Handler *rh1Handler){
    for(int i = 0; i<12; i++){
        ...
        if(curRH1->trackId != 0x42 && curRH1->trackId != 0x43 && curRH1->trackId != 0xff && curRH1->timer != 0){
            CTTP::winningCourse = curRH1->trackId;
