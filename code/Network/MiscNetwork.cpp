...
void DecideCC(RKNetSELECTHandler *handler){
    u8 ccSetting = SIPDataHolder::GetInstance()->GetSettings()->pages[CTTP_HOST_SETTINGS].radioSetting[CTTP_RADIO_CC];
    u8 ccClass = 1; //1 200, 2 150, 3 mirror
    if(ccSetting == CTTP_RADIO_CC_150) ccClass = 2;
    if(handler->mode == ONLINEMODE_PUBLIC_VS || handler->mode == ONLINEMODE_PUBLIC_BATTLE || ccSetting == CTTP_RADIO_CC_NORMAL){
        Random random;
