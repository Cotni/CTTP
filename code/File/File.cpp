...
s32 FileHandler::Open(const char *path, u32 mode){
    ...
    this->fd = CTTP::Open(realPath, (IOS::Mode) mode);
...
s32 RiivoFileHandler::GetDevice_fd() const{
    return CTTP::Open("file", IOS::MODE_NONE);
}
...
void RiivoFileHandler::Delete(){
    s32 riivo_fd = CTTP::Open("file", IOS::MODE_NONE);
