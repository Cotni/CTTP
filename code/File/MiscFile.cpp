...
void LoadUIFile(ArchiveFile *file, const char *path, EGG::Heap *decompressedHeap, bool isCompressed, s32 allocDirection, 
    ...
    if(&ArchiveRoot::sInstance->archivesHolders[ARCHIVE_HOLDER_UI]->archives[2] == file) path = CTTP::UIAssets;
    else if(&ArchiveRoot::sInstance->archivesHolders[ARCHIVE_HOLDER_COMMON]->archives[2] == file) path = CTTP::CommonAssets;
