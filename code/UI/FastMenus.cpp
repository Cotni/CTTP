...
void FasterMenusOnTransition(MenuData *menuData, u32 delay, u32 r5){
    if (SIPDataHolder::GetInstance()->GetSettings()->pages[CTTP_MENU_SETTINGS].radioSetting[CTTP_RADIO_FASTMENUS] == CTTP_RADIO_FASTMENUS_ENABLED) delay = 0;
...
void FasterMenuPatchTransitionDelay(){
    ...
    if (SIPDataHolder::GetInstance()->GetSettings()->pages[CTTP_MENU_SETTINGS].radioSetting[CTTP_RADIO_FASTMENUS] == CTTP_RADIO_FASTMENUS_ENABLED) transitionDelay -= transitionDelay;
...
void FasterMenusOnBoot(){
    float transitionDelay = 176.0f;
    if (SIPDataHolder::GetInstance()->GetSettings()->pages[CTTP_MENU_SETTINGS].radioSetting[CTTP_RADIO_FASTMENUS] == CTTP_RADIO_FASTMENUS_ENABLED){
