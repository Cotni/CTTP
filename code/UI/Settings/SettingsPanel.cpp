...
namespace CTTPUI{
void AddSettingsPage(Scene *scene, PageId id){
    scene->CreatePage(id);
    scene->CreatePage(VS_SETTINGS);
}
...
Page *CreateSettingsPage(){
    ...
    || (id == P2_WIFI) || (id == P2_WIFI_GLOBE_DISCONNECT)){
        ...
        scene->Set(settings, VS_TEAMS_VIEW);
        settings->Setup(VS_TEAMS_VIEW);

        buttonsPerRow[1] = 3;
        optionsPerScroller[0] = 7;
        settings = new(SettingsPanel)(2, 1, buttonsPerRow, optionsPerScroller, 2); //host settings
        scene->Set(settings, BATTLE_MODE_SELECT);
        settings->Setup(BATTLE_MODE_SELECT);
...
SettingsPanel::SettingsPanel(u32 radioButtonsCount, u32 scrollerButtonsCount, const u8 buttonsPerRow[8], const u8 optionsPerScroller[8], u32 sheetId){
    ...
    MenuId id = MenuData::sInstance->curScene->menuId;
    if (id == OPTIONS_MENU) prevPageId = OPTIONS;
    else if ((id == P1_WIFI)||(id == P1_WIFI_GLOBE_DISCONNECT)||(id == P1_WIFI_FROM_FIND_FRIEND)
    || (id == P2_WIFI) || (id == P2_WIFI_GLOBE_DISCONNECT)) prevPageId = WFC_MAIN_PAGE;
    nextMenu = MENU_NONE;
    controlSources = 2;
    ...
    onButtonSelectHandler.ptmf = static_cast<void (Menu::*)(PushButton *, u32)> (&SettingsPanel::OnExternalButtonSelect);
    ...
    onBackPressHandler.ptmf = static_cast<void (Menu::*)(u32)> (&SettingsPanel::OnBackPress);
    onStartPressHandler.subject = this;
    onStartPressHandler.ptmf = (&Menu::HandleStartPress);

    onButtonClickHandler.subject = this;
    onButtonClickHandler.ptmf = static_cast<void (Menu::*)(PushButton *, u32)> (&SettingsPanel::OnSaveButtonClick);
    onRightButtonClickHandler.subject = this;
    onRightButtonClickHandler.ptmf = static_cast<void (Menu::*)(PushButton *, u32)> (&SettingsPanel::OnRightButtonClick);
    onLeftButtonClickHandler.subject = this;
    onLeftButtonClickHandler.ptmf = static_cast<void (Menu::*)(PushButton *, u32)> (&SettingsPanel::OnLeftButtonClick);
...
UIControl* SettingsPanel::CreateExternalControl(u32 id){
    ...
    this->controlCount++;
    button->Load(CTTP::buttonFolder, "Settings", variant, this->activePlayerBitfield, 0, false);
    return button;
    //has BMG 0x7D0 for OK
};

UIControl *SettingsPanel::CreateControl(u32 id){
    SettingsHolder *data = SettingsHolder::GetInstance();
    if (id < this->radioCount){
        ...
        radioButtonControl->Load(this->buttonsPerRow[id], data->GetSettings()->pages[this->sheetIdx].radioSetting[id], CTTP::controlFolder, "SettingsRadioBase", variant, "SettingsRadioOption", optionVariants, 1, 0, 0);
        ...
        upDownControl->Load(this->optionsPerScroller[id], data->GetSettings()->pages[this->sheetIdx].scrollSetting[id], CTTP::controlFolder, "SettingsUpDownBase", variant, "SettingsUpDownButtonR", "RightButton",
        ...
        valueControl->Load(CTTP::controlFolder, "SettingsUpDownValue", "Value", "SettingsUpDownText", "Text");
...
void SettingsPanel::OnActivate(){
    SettingsPage *page = &SettingsHolder::GetInstance()->GetSettings()->pages[this->sheetIdx];
...
void SettingsPanel::LoadPrevMenuAndSaveSettings(PushButton *button){
    ...
    Scene *scene = MenuData::sInstance->curScene;
    if(this->prevPageId == OPTIONS) scene->Get<ExpandedOptions>(OPTIONS)->topSettingsPage = this->pageId;
    else if(this->prevPageId == WFC_MAIN_PAGE) scene->Get<ExpandedWFCMain>(WFC_MAIN_PAGE)->topSettingsPage = this->pageId;
    else if(this->prevPageId == FRIEND_ROOM){
        scene->Get<ExpandedFroom>(WFC_MAIN_PAGE)->topSettingsPage = this->pageId;
        this->nextPageId = PAGE_NONE; //FriendRoom's OnResume is important
    } 
    SettingsPanel::SaveSettings();
}

//On Save Click/Back Press, is called and updates SIPData
void SettingsPanel::SaveSettings(){
    Scene *scene = MenuData::sInstance->curScene;
    
    for(int count = 0; count < SETTINGSPAGECOUNT; count++){
        SettingsPanel *panel = scene->Get<SettingsPanel>((PageId)(VS_SETTINGS + count));     
        SettingsPage *settingsPage = &SettingsHolder::GetInstance()->GetSettings()->pages[panel->sheetIdx];
...
void SettingsPanel::OnRightButtonClick(PushButton *button, u32 hudSlotId){
    ...
    SettingsPanel *nextPanel = MenuData::sInstance->curScene->Get<SettingsPanel>(id);
...
void TextUpDownValueControlwID::SetMessage(u32 optionId){
    ...
    u32 bmgId = BMG_SCROLLER_SETTINGS + (((CTTPUI::SettingsPanel*)this->parentGroup->parentPage)->sheetIdx<<12) + (this->id+1<<4)+ optionId;
    text->SetMsgId(bmgId);
}
}//namespace SIPUI
