...
MenuId BootIntoMenu(){
    SettingsHolder *params = SIPDataHolder::GetInstance();
    MenuId menu = MENU_NONE;
    u8 bootSetting = SIPDataHolder::GetInstance()->GetSettings()->pages[CTTP_MENU_SETTINGS].scrollSetting[CTTP_SCROLL_BOOT];
    u8 license = 0;
    if(bootSetting != CTTP_SCROLL_BOOT_DISABLED){
...
void SetUpCorrectController(RealControllerHolder *realControllerHolder, Controller *controller, u32 loopIndex){  //r4 usually uses inputdata3 which is slot and controller independant
    MenuDataPad *pad = &MenuData::sInstance->pad;
